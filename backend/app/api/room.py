...
async def create_room(room: RoomCreate, current_user: User = Depends(admin_required), db: Session = Depends(get_db)):
    return create_room_logic(db=db, room=room)

# 会議室更新
@router.put("/rooms/{room_id}", response_model=Room)
async def update_room(room_id: int, room: RoomCreate, current_user: User = Depends(admin_required), db: Session = Depends(get_db)):
    print(f"Updating room with ID: {room_id} and data: {room}")
    return update_room_logic(db=db, room=room, room_id=room_id)

# 会議室削除
@router.delete("/rooms/{room_id}")
async def delete_room(room_id: int, current_user: User = Depends(admin_required), db: Session = Depends(get_db)):
